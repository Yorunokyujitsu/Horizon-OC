...
namespace kip {
    ...
    void SetKipData() {
        ...
        if (fp == NULL) {
            notification::writeNotification("KIP 패치 실행 실패!");
            kipAvailable = false;
            return;
        }
        kipAvailable = true;
        ...
        if (!cust_read_table_f(fp, &table)) {
            ...
            notification::writeNotification("KIP 패치 읽기 실패!");
            return;
        }
        ...
        if (custRev < CUST_REV || kipVersion < KIP_VERSION) {
            fclose(fp);
            notification::writeNotification("미지원 KIP 패치 검출\n업데이트 필요!");
            ...
            fclose(fp);
            notification::writeNotification("미지원 버전 모듈 검출\n업데이트 필요!");
            fileUtils::LogLine("Cust revision: %u", custRev);
            fileUtils::LogLine("Kip version: %u", kipVersion);
            return;
        }
        ...
        if (!cust_write_table_f(fp, &table)) {
            ...
            notification::writeNotification("KIP 패치 쓰기 실패!");
            return;
        }
        fclose(fp);
        ...
        if (config::SetConfigValues(&configValues, true)) {
            ...
            notification::writeNotification("KIP 패치 설정 적용 실패!");
        }
    }
    ...
    bool IsKipLoaded() {
        ...
        notification::writeNotification("KIP 패치가 없거나 비활성화됨");
    ...
    void GetKipData() {
        ...
        if ((u64)crc32::checksum_file("sdmc:/atmosphere/kips/loader.kip") != config::GetConfigValue(KipCrc32) &&
            ...
            SetKipData();
            notification::writeNotification("KIP 패치 업데이트됨, 재부팅 필요!");
            return;
        }
        ...
        if (sizeof(HocClkConfigValueList) <= sizeof(configValues)) {
            ...
            notification::writeNotification("설정 버퍼 불일치!");
        }
    }
