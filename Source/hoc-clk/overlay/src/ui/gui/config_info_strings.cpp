...
std::vector<std::string> ConfigInfoStrings(HocClkConfigValue val, bool isMariko, bool isHoag)
{
    switch (val)
    {
        ...
        case HocClkConfigValue_ThermalThrottleThreshold:
            return {
                "온도 제한이 활성화된 상태에서, 기본 클럭으로 복원되기 위한 온도 임계값입니다 (°C 단위).",
                "기본값: 70°C"
            };
        ...
        case KipConfigValue_stepMode:
            return {
                "RAM 클럭이 조절되는 단계값입니다.",
                "옵션:",
                " - 33 MHz: 33 MHz 단위로 변경 (예: 1600, 1633, 1666, 1700 등)",
                " - 66 MHz: 66 MHz 단위로 변경 (예: 1600, 1666, 1733 등)",
                " - 100 MHz: 100 MHz 단위로 변경 (예: 1600, 1700, 1800 등)",
                " - 133 MHz: 133 MHz 단위로 변경 (예: 1600, 1733, 1866 등)",
                " - JEDEC:",
        ...
        case KipConfigValue_read_latency_2133:
            return {
                ...
                "이 값들 중 일부는 생략해도 정상 동작합니다. (예를 들어 1333을 -로 설정하면, 2000 MHz 미만에서도 1600 레이턴시가 사용됩니다.)",
                "이 설정들을 모두 생략하면 레이턴시는 다음과 같이 자동 계산됩니다:",
                "1633-1866 MHz→1866 WRL",
                "1900+ MHz - 2133 WRL",
                "이 설정들은 읽기 및 쓰기 레이턴시에 모두에 적용되며, 필요에 따라 브래킷 값을 서로 조합해서 사용할 수 있습니다.",
                "기본값: ⋯"
            };
        ...
        case KipConfigValue_marikoGpuVmin:
            return {
                "최소 GPU 전압입니다.",
                "참고: DVFS 모드가 PCV 하이재킹으로 설정된 경우, RAM 클럭 변경 시 DVFS에 의해 이 값이 변경될 수 있습니다.",
                "기본값: 610 mV"
            };
        ...
        case HocClkConfigValue_DVFSMode:
            return {
                ...
                "옵션:",
                "- 비활성화: 비활성화됨",
                "- PCV 하이재킹: 오버라이드를 위해 PCV를 하이재킹함",
                "기본값: PCV 하이재킹"
            };

        case HocClkConfigValue_DVFSOffset:
            return {
                "PCV 하이재킹 모드에서 RAM 클럭 변경으로 인해 요구 사양이 높아질 때, GPU 최소 전압에 더하거나 뺄 오프셋 값입니다.",
                "기본값: 0 mV (비활성화)"
            };
        ...
        case HocClkConfigValue_LiveGpuVoltage:
            return {
                "GPU 전압을 실시간으로 조정합니다.",
                "경고: 불안정한 설정도 허용됩니다.",
                "예외: RAM 오버클럭이 활성화된 동안에는 안정성에 필요한 최소 전압 아래로 낮출 수 없습니다.",
                "RAM 주파수를 변경하면 요청한 전압이 덮어써집니다.",
                "전압은 모든 GPU 주파수에 공통으로 적용됩니다.",
                "\"초기화\"는 기본 전압 또는 RAM 안정성에 필요한 최소 전압으로 복원합니다."
            };
        default:
            return {};
    }
